...
**每句话都落在时间轴上，而且边界拖得动。** 不管转写来自 ASR 还是人工录入，保存时都会自动分句（中文按 `。！？…` 与换行、长句再按 `；，、` 细拆）并对齐到音频区间：ASR 有时间戳段就贴着段、段内按发音字符均摊；纯文本则把整条音频按字数均摊，先给一版可用初稿。整理者在波形上拖动句间竖线即可修正边界，也能拆分、合并、改字，保存时服务端强校验"不越界、不倒挂、不重叠、非空"（`PUT /api/audio/:id/sentences`），规则与校验都只有一份（`packages/shared/src/transcript.ts`）。每句话都能单独试听，也能一键带着它的时间区间生成待澄清条目。分句结果存在 `AudioAttachment.sentences`，手工修正标 `source: manual`。
